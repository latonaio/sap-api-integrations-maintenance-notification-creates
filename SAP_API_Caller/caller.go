...
import (
	...
	sap_api_output_formatter "sap-api-integrations-maintenance-notification-creates/SAP_API_Output_Formatter"
	"strings"
	"sync"
...
func (c *SAPAPICaller) Header(header *requests.Header) {
	outputDataHeader, err := c.callMaintenanceNotificationSrvAPIRequirementHeader("MaintenanceNotification", header)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callMaintenanceNotificationSrvAPIRequirementHeader(api string, header *requests.Header) (*sap_api_output_formatter.Header, error) {
...
func (c *SAPAPICaller) Item(item *requests.Item) {
	outputDataItem, err := c.callMaintenanceNotificationSrvAPIRequirementItem("MaintenanceNotificationItem", item)
	if err != nil {
		c.log.Error(err)
		return
	}
...
func (c *SAPAPICaller) callMaintenanceNotificationSrvAPIRequirementItem(api string, item *requests.Item) (*sap_api_output_formatter.Item, error) {
